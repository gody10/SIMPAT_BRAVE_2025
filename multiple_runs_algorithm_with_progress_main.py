...
sarsa_logger = setup_logger('sarsa_logger', 'sarsa_algorithm.log')  
...
for run in tqdm(range(1, NUM_RUNS + 1), desc="Run Progress"):
    ...
    # Run the Q-Brave Algorithm
    success_q_brave_ = algorithm.sarsa_single_agent(
        solving_method="scipy",
        number_of_episodes=NUMBER_OF_EPISODES,
        max_travels_per_episode=MAX_ITER,
        b=B,
    ...
    sarsa_logger.info("The UAV energy level is: %s", algorithm.get_uav().get_energy_level())
    sarsa_logger.info("The UAV processed in total: %s bits", algorithm.most_processed_bits)
    
    # Accumulate metrics
    algorithms_total_bits_acc["SARSA Total Bits"].append(algorithm.get_most_processed_bits())
    algorithms_expended_energy_acc["SARSA Energy Level"].append(algorithm.get_most_expended_energy())
    algorithms_total_visited_nodes_acc["SARSA Total Visited Nodes"].append(len(algorithm.get_most_visited_nodes()))
    
    # End the timer for Q-Brave Algorithm
    q_brave_time = time.time() - start_time
    q_brave_logger.info("SARSA Algorithm took: %s seconds", q_brave_time)
    timer_dict_acc["SARSA Time"].append(q_brave_time)
    
    if success_q_brave_:
        sarsa_logger.info("SARSA Algorithm has successfully reached the final node!")
    else:
        sarsa_logger.info("SARSA Algorithm failed to reach the final node!")
    ...
    if run % SAVE_INTERVAL == 0:
        ...
        system_logger.info("Computing current averages and saving as pickle files...")
        ...
        system_logger.info(f"Current averages up to run {run} have been computed!")
        system_logger.info("Total Bits: %s", current_total_bits_avg)
        system_logger.info("Expended Energy: %s", current_expended_energy_avg)
        system_logger.info("Total Visited Nodes: %s", current_total_visited_nodes_avg)
        system_logger.info("Timer Dict: %s", current_timer_dict_avg)

        # Define filenames with run number
        bits_filename = f"algorithms_total_bits_avg_sarsa.pkl"
        energy_filename = f"algorithms_expended_energy_avg_sarsa.pkl"
        nodes_filename = f"algorithms_total_visited_nodes_avg_sarsa.pkl"
        timer_filename = f"timer_dict_avg_sarsa.pkl"
...
final_bits_filename = "algorithms_total_bits_avg_sarsa.pkl"
final_energy_filename = "algorithms_expended_energy_avg_sarsa.pkl"
final_nodes_filename = "algorithms_total_visited_nodes_avg_sarsa.pkl"
final_timer_filename = "timer_dict_avg_sarsa.pkl"
